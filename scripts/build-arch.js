...
async function main() {
  try {
    ...
    // /opt/zalomarchy
    const optDir = path.join(TEMP_PKG_DIR, 'opt', 'zalomarchy');
    ...
    const mainBin = path.join(optDir, 'zalomarchy');
    ...
    // /usr/bin/zalomarchy launcher script & /usr/bin/zalo symlink
    const binDir = path.join(TEMP_PKG_DIR, 'usr', 'bin');
    fs.mkdirSync(binDir, { recursive: true });
    const launcherSrc = path.join(BASE_DIR, 'packaging', 'arch', 'zalomarchy.sh');
    const launcherDest = path.join(binDir, 'zalomarchy');
    fs.copyFileSync(launcherSrc, launcherDest);
    fs.chmodSync(launcherDest, 0o755);
    try {
      fs.symlinkSync('/usr/bin/zalomarchy', path.join(binDir, 'zalo'));
    } catch (_) {}

    // /usr/share/applications/zalomarchy.desktop
    const appsDir = path.join(TEMP_PKG_DIR, 'usr', 'share', 'applications');
    fs.mkdirSync(appsDir, { recursive: true });
    fs.copyFileSync(
      path.join(BASE_DIR, 'packaging', 'arch', 'zalomarchy.desktop'),
      path.join(appsDir, 'zalomarchy.desktop')
    );
    ...
    for (const size of iconSizes) {
      ...
      if (iconSrc) {
        ...
        fs.copyFileSync(iconSrc, path.join(destDir, 'zalomarchy.png'));
    ...
    if (fs.existsSync(icon512)) {
      ...
      fs.copyFileSync(icon512, path.join(pixmapsDir, 'zalomarchy.png'));
      fs.copyFileSync(icon512, path.join(pixmapsDir, 'zalo.png'));
    }

    // License
    const licenseDir = path.join(TEMP_PKG_DIR, 'usr', 'share', 'licenses', 'zalomarchy');
    ...
    const pkgInfoContent = [
      '# Generated by zalomarchy build-arch.js',
      'pkgname = zalomarchy',
      'pkgbase = zalomarchy',
    ...
    const installScriptletSrc = path.join(BASE_DIR, 'packaging', 'arch', 'zalomarchy.install');
    ...
    const pkgName = `zalomarchy-${ZALO_VERSION}-1-x86_64.pkg.tar.zst`;
